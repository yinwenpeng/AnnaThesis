...
from preprocess_socher_guu import neg_entity_tensor_v2
from common_functions import cosine_matrix1_matrix2_rowwise, cosine_tensors, Conv_with_input_para, LSTM_Batch_Tensor_Input_with_Mask, create_conv_para,create_GRU_para, GRU_Batch_Tensor_Input_with_Mask, create_LSTM_para
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=6, nn='LSTM', rel_emb_size=30, margin=0.3, batch_size=50, maxSentLen=5, neg_size=10, filter_size=3):
...
#                                       neg_entity_tensor(ent_idmatrix, rel_idmatrix, tuple2tailset, neg_size, ent_vocab_set))
            ...
            if epoch >=6 and iter%25000==0:
                ...
                for idd, dev_batch_id in enumerate(dev_batch_start): # for each test batch
                    ...
                    dev_ent_idmatrix=dev_ents_store[dev_batch_id:dev_batch_id+batch_size]
                    dev_rel_idmatrix=dev_paths_store[dev_batch_id:dev_batch_id+batch_size]
                    dev_ground_ent_idlist=dev_ent_idmatrix[:,-1]
                    dev_masks_matrix=dev_masks_store[dev_batch_id:dev_batch_id+batch_size]
                    cosine_batch_vocab=dev_model(
                                       dev_ent_idmatrix[:,0],
                                       dev_rel_idmatrix,
                                       dev_masks_matrix)
#                                       test_ent_idmatrix[:,1:])
                    ...
                    for i in range(batch_size):
                        ground_id=dev_ground_ent_idlist[i]
                        mask_sum=np.sum(dev_masks_matrix[i])

                        if mask_sum==1.0:
                            head_id=dev_ent_idmatrix[i][0]
                        else:
                            head_id=dev_ent_idmatrix[i][-2]
                        pair=(head_id, dev_rel_idmatrix[i][-1])
                        filted_idset=tuple2tailset.get(pair)
                        focus_idset=rel2tailset.get(dev_rel_idmatrix[i][-1])
                        if filted_idset is None:
                            print pair, 'is not in the training set'
                            print idd*batch_size+i
                            print dev_ent_idmatrix[i], dev_rel_idmatrix[i]
                            exit(0)
                ...
                dev_hit10=dev_succ/len(dev_batch_start)
                if dev_hit10 > max_acc_dev:
                    max_acc_dev=dev_hit10 
                    print 'current dev_hit10:', dev_hit10, '\t\t\t\t\tmax dev_hit10:', max_acc_dev
                    #test
                    succ=0
